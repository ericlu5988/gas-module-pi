...
class MQ():
    ...
    CALIBRATION_SAMPLE_INTERVAL  = 200      # define the time interval(in milisecond) between each samples in the
                                            # cablibration phase
    ...
    def MQResistanceCalculation(self, raw_adc):
        print(f"RAW ADC: {raw_adc}                RL_VALUE: {self.RL_VALUE}")   
    ...
    def MQCalibration(self, mq_pin):
        ...
        for i in range(self.CALIBARAION_SAMPLE_TIMES):          # take multiple samples
            raw_reading = self.adc.read(mq_pin)
            if raw_reading!=0:
        ...
        val = val/(self.CALIBARAION_SAMPLE_TIMES - unsuccess)                 # calculate the average value

        val = val/self.RO_CLEAN_AIR_FACTOR                      # divided by RO_CLEAN_AIR_FACTOR yields the Ro 
                                                                # according to the chart in the datasheet 
        print("Success times: {self.CALIBRATION_SAMPLE_TIMES - unsuccess} ..... Unsuccess times: {unsuccess}")
        return val;
...
#        print(f"PCURVE 0: {pcurve[0]} ..... PCURVE 1: {pcurve[1]} ..... PCURVE 2: {pcurve[2]}")
